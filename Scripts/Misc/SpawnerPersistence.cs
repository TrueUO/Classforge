...
namespace Server
{
    public class SpawnerPersistence
    {
        ...
        public enum SpawnerVersion
        {
            ...
            RemoveUnused = 0x00000010,
            RemoveUnused2 = 0x00000020,
            RemoveTeleporters = 0x00000040,
            DestardSpawners = 0x00000080,
            DoomSpawners = 0x00000100
        }
        ...
        public static void RemoveTeleporters()
        {
            ...
            Timer.DelayCall(TimeSpan.FromSeconds(1), () =>
            {
                ...
                for (int index = 0; index < Region.Regions.Count; index++)
                {
                    ...
                    {
                        for (int i = 0; i < reg.Area.Length; i++)
                        {
                            Rectangle3D rec = reg.Area[i];
        ...
        public static void Delete(string current)
        {
            ...
            {
                for (int index = 0; index < spawner.SpawnObjects.Length; index++)
                {
        ...
        public static bool Replace(ISpawner spwner, string current, string replace, string check)
        {
            ...
            {
                for (int index = 0; index < xmlSpawner.SpawnObjects.Length; index++)
                {
                    ...
                    if (files.Count > 0)
                    {
                        ...
                        for (int i = 0; i < files.Count; i++)
                        {
                            ...
                            if (ds.Tables != null && ds.Tables.Count > 0)
                            {
                                ...
                                {
                                    for (int index = 0; index < ds.Tables["Points"].Rows.Count; index++)
                                    {
                    ...
                    if (files.Count > 0)
                    {
                        ...
                        for (int index = 0; index < files.Count; index++)
                        {
                            ...
                            if (ds.Tables != null && ds.Tables.Count > 0)
                            {
                                if (ds.Tables["Points"] != null && ds.Tables["Points"].Rows.Count > 0)
                                {
                                    for (int i = 0; i < ds.Tables["Points"].Rows.Count; i++)
                                    {
