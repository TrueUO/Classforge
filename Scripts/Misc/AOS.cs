...
namespace Server
{
    ...
    public class AOS
    {
        ...
        public static int Damage(IDamageable damageable, Mobile from, int damage, bool ignoreArmor, int phys, int fire, int cold, int pois, int nrgy, int chaos, int direct, bool keepAlive, DamageType type = DamageType.Melee)
        {
            ...
            BaseCreature toCreature = m as BaseCreature;
        ...
        public static int GetStatus(Mobile from, int index)
        {
            switch (index)
            {
                ...
                case 5:
                {
                    return Math.Min(45, AosAttributes.GetValue(from, AosAttribute.DefendChance));
                }
                case 6:
                {
                    return 45 + WhiteTigerFormSpell.GetDefenseCap(from);
                }
                case 7:
                {
                    return Math.Min(45, AosAttributes.GetValue(from, AosAttribute.AttackChance));
                }
                case 8:
                {
                    return Math.Min(60, AosAttributes.GetValue(from, AosAttribute.WeaponSpeed));
                }
                case 9:
                {
                    return Math.Min(100, AosAttributes.GetValue(from, AosAttribute.WeaponDamage));
                }
                case 10:
                {
                    return Math.Min(100, AosAttributes.GetValue(from, AosAttribute.LowerRegCost));
                }
                case 11:
                {
                    return AosAttributes.GetValue(from, AosAttribute.SpellDamage);
                }
                case 12:
                {
                    return Math.Min(6, AosAttributes.GetValue(from, AosAttribute.CastRecovery));
                }
                case 13:
                {
                    return Math.Min(4, AosAttributes.GetValue(from, AosAttribute.CastSpeed));
                }
                case 14:
                {
                    return AosAttributes.GetValue(from, AosAttribute.LowerManaCost);
                }

                case 15:
                {
                    return (int)RegenRates.HitPointRegen(from); // HP   REGEN
                }
                case 16:
                {
                    return (int)RegenRates.StamRegen(from); // Stam REGEN
                }
                case 17:
                {
                    return (int)RegenRates.ManaRegen(from); // MANA REGEN
                }
                case 18:
                {
                    return Math.Min(105, AosAttributes.GetValue(from, AosAttribute.ReflectPhysical)); // reflect phys
                }
                case 19:
                {
                    return Math.Min(50, AosAttributes.GetValue(from, AosAttribute.EnhancePotions)); // enhance pots
                }
    ...
    public sealed class AosAttributes : BaseAttributes
    {
        ...
        public static int[] GetValues(Mobile m, params AosAttribute[] attributes)
        {
            ...
            foreach (int value in EnumerateValues(m, attributes))
            {
                list.Add(value);
            }
        ...
        {
            foreach (AosAttribute a in attributes)
            {
    ...
    public enum AosWeaponAttribute
    {
        ...
        BloodDrinker = 0x01000000,
        BattleLust = 0x02000000,
        HitCurse = 0x04000000,
        HitFatigue = 0x08000000,
        HitManaDrain = 0x10000000,
        ReactiveParalyze = 0x20000000
    }
    ...
    public sealed class AosWeaponAttributes : BaseAttributes
    {
        ...
        {
            foreach (AosWeaponAttribute a in attributes)
            {
                yield return GetValue(m, a);
            }
        }
    ...
    public sealed class AosArmorAttributes : BaseAttributes
    {
        ...
        {
            foreach (AosArmorAttribute a in attributes)
            {
                yield return GetValue(m, a);
            }
        }
    ...
    public sealed class SAAbsorptionAttributes : BaseAttributes
    {
        ...
        {
            foreach (SAAbsorptionAttribute a in attributes)
            {
                yield return GetValue(m, a);
            }
        }
