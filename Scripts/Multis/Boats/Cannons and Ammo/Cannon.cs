...
namespace Server.Items
{
    ...
    public abstract class BaseCannon : Item, IShipCannon
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            else if (m_Galleon.GetSecurityLevel(from) >= SecurityLevel.Crewman)
            {
                ...
                switch (m_DamageState)
                {
                    ...
                    {
                        from.SendLocalizedMessage(1116606); break;//The cannon is lightly damaged and needs some minor repair.
                    }
                    case DamageLevel.Moderately:
                    {
                        from.SendLocalizedMessage(1116607); break;//The cannon is moderately damaged and needs some repairs. This cannon is safe to fire.
                    }
                    case DamageLevel.Severely:
                    {
                        from.SendLocalizedMessage(1116608); break;//The cannon is severely damaged. It needs major repairs before it is safe to fire.
                    }
                }
            }
            else
        ...
        public virtual void OnMobileHit(object obj)
        {
            ...
            {
                for (int index = 0; index < mobsToHit.Count; index++)
                {
        ...
        {
            for (int index = 0; index < LoadTypes.Length; index++)
            {
