...
namespace Server.Engines.ArtisanFestival
{
    public class SpecialGift : BaseContainer
    {
        ...
        private Item RandomGift(Mobile m)
        {
            switch (Utility.Random(5))
            {
                default:
                {
                    return new RewardLantern(m);
                }
                case 1:
                {
                    return new RewardPillow(m);
                }
                case 2:
                {
                    return new RewardPainting(m);
                }
                case 3:
                {
                    return new RewardSculpture(m);
                }
                case 4:
                {
                    return new RewardStainedGlassWindow(m);
                }
            }
        }
    ...
    public class FestivalGiftBox : Item
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                SpecialGift gift = new SpecialGift(from)
                {
                    Hue = Hue
                };
