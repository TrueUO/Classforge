...
namespace Server.Items
{
    public class SkullRugAddon : BaseAddon
    {
        ...
        public override BaseAddonDeed Deed => new SkullRugAddonDeed(RugType, m_ResourceCount, NextResourceCount);
        ...
        public SkullRugAddon(RugType type, int resCount, DateTime nextuse)
        {
            ...
            switch (type)
            {
                ...
                {
                    list = _EastLarge; break;
                }
                case RugType.SouthLarge:
                {
                    list = _SouthLarge; break;
                }
                case RugType.EastSmall:
                {
                    list = _EastSmall; break;
                }
                case RugType.SouthSmall:
                {
                    list = _SouthSmall; break;
                }
            }
            ...
            if (house != null && (house.IsOwner(from) || house.LockDowns.ContainsKey(this) && house.LockDowns[this] == from))
            {
                if (m_ResourceCount > 0)
                {
                    ...
                    switch (choose)
                    {
                        ...
                        case 0:
                        {
                            facet = Map.Trammel; break;
                        }
                        case 1:
                        {
                            facet = Map.Ilshenar; break;
                        }
                        case 2:
                        {
                            facet = Map.Malas; break;
                        }
                        case 3:
                        {
                            facet = Map.Felucca; break;
                        }
                        case 4:
                        {
                            facet = Map.Tokuno; break;
                        }
                        case 5:
                        {
                            facet = Map.TerMur; break;
                        }
                    }
    ...
    {
        public override BaseAddon Addon => new SkullRugAddon(RugType, m_ResourceCount, NextResourceCount);
