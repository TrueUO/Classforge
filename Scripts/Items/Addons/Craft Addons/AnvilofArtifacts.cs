    ...
    public class AnvilofArtifactsGump : BaseGump
    {
        ...
        public override void OnResponse(RelayInfo info)
        {
            ...
            if (!Entry.Ready)
            {
                ...
                for (int i = 0; i < _ResistOrder.Length; i++)
                {
                    TextRelay textEntry = info.GetTextEntry(index);

                    if (textEntry != null)
                    {
                        int value = Utility.ToInt32(textEntry.Text);
            ...
            if (info.ButtonID == 1)
            {
                ...
                else
                {
                    ...
                    foreach (KeyValuePair<ResistanceType, int> x in Entry.Exceptional)
                    {
                    ...
                    foreach (KeyValuePair<ResistanceType, int> x in Entry.Runic)
                    {
                        runicSum += x.Value;
                    }
