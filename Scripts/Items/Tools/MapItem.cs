    ...
    public class MapItem : Item, ICraftable
    {
        ...
        private static void OnMapCommand(NetState state, PacketReader pvSrc)
        {
            ...
            switch (command)
            {
                case 1:
                {
                    map.OnAddPin(from, x, y); break;
                }
                case 2:
                {
                    map.OnInsertPin(from, number, x, y); break;
                }
                case 3:
                {
                    map.OnChangePin(from, number, x, y); break;
                }
                case 4:
                {
                    map.OnRemovePin(from, number); break;
                }
                case 5:
                {
                    map.OnClearPins(from); break;
                }
                case 6:
                {
                    map.OnToggleEditable(from); break;
                }
            }
        }
