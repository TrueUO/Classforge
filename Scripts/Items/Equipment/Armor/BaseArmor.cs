...
namespace Server.Items
{
    public abstract class BaseArmor : Item, IScissorable, ICraftable, IWearableDurability, IResource, ISetItem, IVvVItem, IOwnerRestricted, ITalismanProtection, IEngravable, IArtifact, ICombatEquipment, IQuality
    {
        ...
        public virtual CharacterClass ClassRequired => CharacterClass.None;
        ...
        */
        ...
        public virtual bool Scissor(Mobile from, Scissors scissors)
        {
            ...
            if (item != null && item.Resources.Count == 1 && item.Resources.GetAt(0).Amount >= 2)
            {
                try
                {
                    Type type = MaterialType == AMT.Cloth ? typeof(Cloth) : CraftResources.GetInfo(m_Resource).ResourceTypes[0];
        ...
        private enum SaveFlag
        {
            ...
            MaxHitPoints = 0x00000080,
            HitPoints = 0x00000100,
            Crafter = 0x00000200,
            Quality = 0x00000400,
            Resource = 0x00000800,
            BaseArmor = 0x00001000,
            StrBonus = 0x00002000,
            DexBonus = 0x00004000,
            IntBonus = 0x00008000,
            StrReq = 0x00010000,
            DexReq = 0x00020000,
            IntReq = 0x00040000,
            MedAllowance = 0x00080000,
            SkillBonuses = 0x00100000,
            PlayerConstructed = 0x00200000,
            xAbsorptionAttributes = 0x00400000,
            xWeaponAttributes = 0x00800000,
            NegativeAttributes = 0x01000000,
        ...
        public override void AddNameProperty(ObjectPropertyList list)
        {
            ...
            switch (m_Resource)
            {
                case CraftResource.DullCopper:
                {
                    oreType = 1053108; break; // dull copper
                }
                case CraftResource.ShadowIron:
                {
                    oreType = 1053107; break; // shadow iron
                }
                case CraftResource.Copper:
                {
                    oreType = 1053106; break; // copper
                }
                case CraftResource.Bronze:
                {
                    oreType = 1053105; break; // bronze
                }
                case CraftResource.Gold:
                {
                    oreType = 1053104; break; // golden
                }
                case CraftResource.Agapite:
                {
                    oreType = 1053103; break; // agapite
                }
                case CraftResource.Verite:
                {
                    oreType = 1053102; break; // verite
                }
                case CraftResource.Valorite:
                {
                    oreType = 1053101; break; // valorite
                }
                case CraftResource.SpinedLeather:
                {
                    oreType = 1061118; break; // spined
                }
                case CraftResource.HornedLeather:
                {
                    oreType = 1061117; break; // horned
                }
                case CraftResource.BarbedLeather:
                {
                ...
                {
                    oreType = 1060814; break; // red
                }
                case CraftResource.YellowScales:
                {
                    oreType = 1060818; break; // yellow
                }
                case CraftResource.BlackScales:
                {
                    oreType = 1060820; break; // black
                }
                case CraftResource.GreenScales:
                {
                    oreType = 1060819; break; // green
                }
                case CraftResource.WhiteScales:
                {
                    oreType = 1060821; break; // white
                }
                case CraftResource.BlueScales:
                {
                    oreType = 1060815; break; // blue
                }
                case CraftResource.OakWood:
                {
                    oreType = 1072533; break; // oak
                }
                case CraftResource.AshWood:
                {
                    oreType = 1072534; break; // ash
                }
                case CraftResource.YewWood:
                {
                    oreType = 1072535; break; // yew
                }
                case CraftResource.Heartwood:
                {
                    oreType = 1072536; break; // heartwood
                }
                case CraftResource.Bloodwood:
                {
                    oreType = 1072538; break; // bloodwood
                }
                case CraftResource.Frostwood:
                {
                    oreType = 1072539; break; // frostwood
                }
                default:
                {
                    oreType = 0; break;
        ...
        public override void AddNameProperties(ObjectPropertyList list)
        {
            ...
            if (ClassRequired != CharacterClass.None)
            {
                switch (ClassRequired)
                {
                    case CharacterClass.Mage:
                    {
                        list.Add(1060659, $"Class Required\t{MageClass.Name}"); // ~1_val~: ~2_val~
                        break;
                    }
                    case CharacterClass.Rogue:
                    {
                        list.Add(1060659, $"Class Required\t{RogueClass.Name}"); // ~1_val~: ~2_val~
                        break;
                    }
                    case CharacterClass.Warrior:
                    {
                        list.Add(1060659, $"Class Required\t{WarriorClass.Name}"); // ~1_val~: ~2_val~
                        break;
                    }
                }
            }
        ...
        {
            AnvilOfArtifactsEntry anvilEntry = CraftContext.GetAnvilEntry(from, false);

            if (anvilEntry != null && anvilEntry.Ready)
            {
                Dictionary<ResistanceType, int> table = runic ? anvilEntry.Runic : anvilEntry.Exceptional;

                foreach (KeyValuePair<ResistanceType, int> kvp in table)
                {
                    switch (kvp.Key)
                    {
                        case ResistanceType.Physical:
                        {
                            m_PhysicalBonus += kvp.Value; break;
                        }
                        case ResistanceType.Fire:
                        {
                            m_FireBonus += kvp.Value; break;
                        }
                        case ResistanceType.Cold:
                        {
                            m_ColdBonus += kvp.Value; break;
                        }
                        case ResistanceType.Poison:
                        {
                            m_PoisonBonus += kvp.Value; break;
                        }
                        case ResistanceType.Energy:
                        {
                            m_EnergyBonus += kvp.Value; break;
                        }
                    }
                }
        ...
        public virtual void DistributeMaterialBonus(CraftAttributeInfo attrInfo)
        {
            ...
            else
            {
                switch (Utility.Random(4))
                {
                    case 0:
                    {
                        m_AosAttributes.WeaponDamage += attrInfo.ArmorDamage; break;
                    }
                    case 1:
                    {
                        m_AosAttributes.AttackChance += attrInfo.ArmorHitChance; break;
                    }
                    case 2:
                    {
                        m_AosAttributes.Luck += attrInfo.ArmorLuck; break;
                    }
                    case 3:
                    {
                        m_AosArmorAttributes.LowerStatReq += attrInfo.ArmorLowerRequirements; break;
                    }
                }
            }
        }
        ...
        public int SetResistBonus(ResistanceType resist)
        {
            ...
            else
            {
                switch (resist)
                {
                    case ResistanceType.Physical:
                    {
                        return m_SetEquipped ? LastEquipped ? (PhysicalResistance * Pieces) + m_SetPhysicalBonus : 0 : PhysicalResistance;
                    }
                    case ResistanceType.Fire:
                    {
                        return m_SetEquipped ? LastEquipped ? (FireResistance * Pieces) + m_SetFireBonus : 0 : FireResistance;
                    }
                    case ResistanceType.Cold:
                    {
                        return m_SetEquipped ? LastEquipped ? (ColdResistance * Pieces) + m_SetColdBonus : 0 : ColdResistance;
                    }
                    case ResistanceType.Poison:
                    {
                        return m_SetEquipped ? LastEquipped ? (PoisonResistance * Pieces) + m_SetPoisonBonus : 0 : PoisonResistance;
                    }
                    case ResistanceType.Energy:
                    {
                        return m_SetEquipped ? LastEquipped ? (EnergyResistance * Pieces) + m_SetEnergyBonus : 0 : EnergyResistance;
                    }
                }
            }
