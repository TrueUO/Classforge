...
namespace Server.Items
{
    ...
    public class RecipeBook : Item, ISecurable
    {
        ...
        public void LoadDefinitions()
        {
            ...
            {
                RecipeScrollDefinition definition = Definitions[index];
            ...
            for (int index = 0; index < list.Count; index++)
            {
                ...
                {
                    RecipeScrollDefinition definition = Definitions[i];

                    defList.Add(definition);
                }

                RecipeScrollDefinition x = defList[index];
        ...
        public override bool OnDragDrop(Mobile from, Item dropped)
        {
            ...
            if (dropped is RecipeScroll recipe)
            {
                ...
                {
                    RecipeScrollDefinition x1 = Recipes[index];

                    if (x1.RecipeID == recipe.RecipeID)
                    {
                        for (int i = 0; i < Recipes.Count; i++)
                        {
                            RecipeScrollDefinition x = Recipes[i];
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            for (int index = 0; index < Recipes.Count; index++)
            {
                RecipeScrollDefinition s = Recipes[index];
                writer.Write(s.ID);
        ...
        public override void GetProperties(ObjectPropertyList list)
        {
            ...
            for (int index = 0; index < Recipes.Count; index++)
            {
                RecipeScrollDefinition x = Recipes[index];
