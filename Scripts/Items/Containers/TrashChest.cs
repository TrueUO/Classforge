    ...
    public class TrashChest : BaseTrash
    {
        ...
        public void Empty()
        {
            ...
            if (items.Count > 0)
            {
                ...
                {
                    CleanupArray x1 = m_Cleanup[i];

                    if (x1.mobiles != null)
                    {
                        HashSet<Mobile> set = new HashSet<Mobile>();

                        for (int index1 = 0; index1 < m_Cleanup.Count; index1++)
                        {
                            CleanupArray x2 = m_Cleanup[index1];
                            Mobile m = x2.mobiles;
                            ...
                            if (set.Add(m) && m_Cleanup.Find(x => x.mobiles == m && x.confirm) != null)
                            {
                                ...
                                for (int index = 0; index < m_Cleanup.Count; index++)
                                {
                                    CleanupArray x = m_Cleanup[index];
                                ...
                                for (int index = 0; index < m_Cleanup.Count; index++)
                                {
                                    CleanupArray r = m_Cleanup[index];
