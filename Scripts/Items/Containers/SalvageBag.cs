...
namespace Server.Items
{
    public class SalvageBag : Bag
    {
        ...
        private void SalvageIngots(Mobile from)
        {
            ...
            for (int index = 0; index < from.Backpack.Items.Count; index++)
            {
                Item i = from.Backpack.Items[index];
        ...
        private void SalvageCloth(Mobile from)
        {
            ...
            Item[] findItems = FindItemsByType(typeof(Item), true);

            for (int index = 0; index < findItems.Length; index++)
            {
