    ...
    public class TotemOfChromaticFortune : Item
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            if (IsChildOf(from.Backpack))
            {
                ...
                {
                    TotemOfChromaticFortuneAddon totem = new TotemOfChromaticFortuneAddon(from);
        ...
        private bool CheckSpawn(Point3D p)
        {
            ...
            foreach (Item item in eable)
            {
                ...
                {
                    foreach (XmlSpawner.SpawnObject so in xmlspawn.SpawnObjects)
                    {
                ...
                {
                    foreach (SpawnObject so in spawn.SpawnObjects)
                    {
                        foreach (Type t in m_Types)
                        {
