...
namespace Server.AccountVault
{
    ...
    public class AccountVault : Container
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            else if (HasAccess(from) && Container != null && from.NetState != null)
            {
                ...
                Timer.DelayCall(TimeSpan.FromMilliseconds(250), () =>
                {
                    ...
                    VaultManager manager = FindNearest<VaultManager>(pm, m => m.InRange(pm.Location, Core.GlobalUpdateRange));
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            if (version == 0)
            {
                Timer.DelayCall(() =>
                {
                    if (Account != null)
                    {
                        AccountVaultContainer cont = Container;
        ...
        internal void ClearVault()
        {
            if (_Container != null)
            {
                AuctionVault auctionVault = AuctionVault.GetFirstAvailable();

                if (auctionVault != null)
                {
                    Auction auction = auctionVault.Auction;
                    ...
                    Mobile m = GetAccountMobile();

                    if (m != null)
                    {
                        string region = Region.Find(GetWorldLocation(), Map).Name;
        ...
        internal void ClaimVault(Mobile m)
        {
            ...
            AccountVaultContainer cont = Container;
            List<Item> items = new List<Item>(Items.Where(i => i != cont && !i.IsChildOf(cont)));
        ...
        private Mobile GetAccountMobile()
        {
            ...
            IAccount account = Accounts.GetAccount(Account);
        ...
        {
            Region old = Region.Find(oldLocation, Map);
        ...
        public void AssignIndex()
        {
            ...
            List<AccountVault> list = new List<AccountVault>();

            for (int i = 0; i < Vaults.Count; i++)
            {
                AccountVault vault = Vaults[i];
        ...
        public static AccountVault GetVault(Mobile m)
        {
            ...
            string reg = Region.Find(m.Location, m.Map).Name;
                ...
                if (vault != null)
                {
                    ...
                    else if (!SystemSettings.HasBalance(pm))
                    {
                        ...
                        if (SystemSettings.UseTokens)
                        {
                            ...
                            StoreEntry entry = UltimaStore.GetEntry(typeof(VaultToken));
        ...
        {
            Region reg = Region.Find(v.GetWorldLocation(), v.Map);

            for (int index = 0; index < SystemSettings.VaultRegions.Length; index++)
            {
                string r = SystemSettings.VaultRegions[index];
