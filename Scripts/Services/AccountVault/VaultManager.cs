...
namespace Server.AccountVault
{
    public class VaultManager : Banker
    {
        ...
        public override void AddCustomContextEntries(Mobile from, List<ContextMenuEntry> list)
        {
            ...
            {
                AccountVault vault = AccountVault.GetVault(from);
                bool inRange = vault != null && from.Region.IsPartOf(this.Region);

                OpenVaultEntry open = new OpenVaultEntry(this)
                {
                    Enabled = vault != null && inRange && !from.Criminal
                };
                list.Add(open);

                RentVaultEntry rent = new RentVaultEntry(this)
                {
                    Enabled = vault == null
                };
                list.Add(rent);

                ClaimVaultEntry claim = new ClaimVaultEntry(this)
                {
                    Enabled = vault != null && inRange
                };
                list.Add(claim);

                VaultActionsEntry actions = new VaultActionsEntry(this, vault)
                {
                    Enabled = vault != null && inRange
                };
                list.Add(actions);

                VaultLocationsEntry locations = new VaultLocationsEntry(this);
                list.Add(locations);
            }
        }
        ...
        public class ClaimVaultEntry : ContextMenuEntry
        {
            ...
            public override void OnClick()
            {
                if (Owner.From is PlayerMobile pm)
                {
                    AccountVault vault = AccountVault.GetVault(pm);
