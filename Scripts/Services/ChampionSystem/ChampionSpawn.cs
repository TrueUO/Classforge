...
namespace Server.Engines.CannedEvil
{
    public class ChampionSpawn : Item
    {
            ...
            else
            {
                ...
                for (int i = 0; i < m_Creatures.Count; ++i)
                {
                    ...
                    if (m.Deleted)
                    {
                        ...
                        {
                            if (Map == Map.Felucca && Utility.RandomDouble() < ChampionSystem.ScrollChance)
                            {
        ...
        public void Respawn()
        {
            ...
            for (int index = 0; index < m_Creatures.Count; index++)
            {
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            bool restarting = RestartTimerRunning;
        ...
        private class ChampionSpawnInfoGump : Gump
        {
            ...
            static ChampionSpawnInfoGump()
            {
                ...
                for (int index = 0; index < gWidths.Length; index++)
                {
                    int width = gWidths[index];
                ...
                : base(40, 40)
            {
                ...
                foreach (Damager damager in damagers.OrderByDescending(x => x.Damage))
                {
                    damagers.Add(damager);
                }

                for (int index = 0; index < damagers.Count; index++)
                {
