...
namespace Server.Engines.ArenaSystem
{
    public class PVPArenaSystem : PointsSystem
    {
        ...
        public void OnTick()
        {
            for (int index = 0; index < Arenas.Count; index++)
            {
                PVPArena a = Arenas[index];
        ...
        public ArenaDuel GetBookedDuel(PlayerMobile pm)
        {
            for (int index = 0; index < Arenas.Count; index++)
            {
                ...
                if (arena.BookedDuels.Count > 0)
                {
                    for (int i = 0; i < arena.BookedDuels.Count; i++)
                    {
        ...
        private ArenaDefinition GetDefinition(string name)
        {
            ...
            for (int index = 0; index < ArenaDefinition.Definitions.Length; index++)
            {
                ArenaDefinition d = ArenaDefinition.Definitions[index];
        ...
        public void CheckTitle(PlayerMobile pm)
        {
            ...
            switch (entry.TotalDuels)
            {
                case 1:
                {
                    title = 1152068 + (int)ArenaTitle.FledglingGladiator; break;
                }
                case 50:
                {
                    title = 1152068 + (int)ArenaTitle.BuddingGladiator; break;
                }
                case 100:
                {
                    title = 1152068 + (int)ArenaTitle.Gladiator; break;
                }
                case 250:
                {
                    title = 1152068 + (int)ArenaTitle.WellKnownGladiator; break;
                }
                case 500:
                {
                    title = 1152068 + (int)ArenaTitle.VeteranGladiator; break;
                }
            }
