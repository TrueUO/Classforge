...
namespace Server.Engines.ArenaSystem
{
    ...
    public class ArenaDuel
    {
        ...
        {
            for (int index = 0; index < Arena.Definition.EffectAreas.Length; index++)
            {
                Rectangle2D rec = Arena.Definition.EffectAreas[index];

                List<Item> list = new List<Item>(Arena.Blockers);
        ...
        {
            for (int index = 0; index < Arena.Blockers.Count; index++)
            {
            ...
            foreach (KeyValuePair<PlayerMobile, PlayerStatsEntry> kvp in GetParticipants(true))
            {
                ...
                if (winner == null)
                {
                    switch (BattleMode)
                    {
                        ...
                        {
                            stats.SurvivalDraws++; break;
                        }
                        case BattleMode.Team:
                        {
                            stats.TeamDraws++; break;
                        }
                    }
                }
                else if (team == winner)
                {
                    switch (BattleMode)
                    {
                        case BattleMode.Survival:
                        {
                            stats.SurvivalWins++; break;
                        }
                        case BattleMode.Team:
                        {
                            stats.TeamWins++; break;
                        }
                    }
                }
                else
                {
                    switch (BattleMode)
                    {
                        case BattleMode.Survival:
                        {
                            stats.SurvivalLosses++; break;
                        }
                        case BattleMode.Team:
                        {
                            stats.TeamLosses++; break;
                        }
                    }
                }
            }
        ...
        public bool CheckTeamAlive(PlayerMobile justDied, ArenaTeam team)
        {
            ...
            foreach (PlayerMobile p in team.Players.Keys)
            {
        ...
        public void Serialize(GenericWriter writer)
        {
            ...
            for (int index = 0; index < Warned.Count; index++)
            {
