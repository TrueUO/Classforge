...
namespace Server.Items
{
    public class TreasureMapChest : LockableContainer
    {
        ...
        public static void Fill(Mobile from, LockableContainer cont, int level, bool isSos)
        {
            ...
            else
            {
                ...
                if (isSos)
                {
                    switch (level)
                    {
                        ...
                        case 1:
                        {
                            count = Utility.RandomMinMax(2, 5); break;
                        }
                        case 2:
                        {
                            count = Utility.RandomMinMax(10, 15); break;
                        }
                    }
                }
                else
            ...
            if (isSos)
            {
                switch (level)
                {
                    default:
                    {
                        count = Utility.RandomMinMax(45, 60); break;
                    }
                    case 0:
                    case 1:
                    {
                        count = Utility.RandomMinMax(15, 20); break;
                    }
                    case 2:
                    {
                        count = Utility.RandomMinMax(25, 40); break;
                    }
                }
            }
            else
        ...
        public virtual bool CanOpen(Mobile from)
        {
            ...
            for (int index = 0; index < AncientGuardians.Count; index++)
            {
                Mobile ag = AncientGuardians[index];
        ...
        public void SpawnAncientGuardian(Mobile from)
        {
            ...
            for (int index = 0; index < AncientGuardians.Count; index++)
            {
                Mobile g = AncientGuardians[index];
