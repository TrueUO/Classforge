...
{
    public static class ClericClass
    {
        public const string Name = "Cleric";

        public static readonly SkillName[] PrimarySkills =
        [
            SkillName.Healing, SkillName.Macing
        ];
        ...
        {
            // +10 Int/Dex bonus to start
            pm.RawInt += 10;
            pm.RawDex += 10;

            // starting gear
            pm.EquipItem(new GnarledStaff());
            pm.EquipItem(new Shoes());

            pm.AddToBackpack(new Bandage(50));
        }
    }
}
