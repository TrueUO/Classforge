        ...
        public GuildLeaderboardGump(PlayerMobile pm, Filter filter = Filter.Score) : base(50, 50)
        {
            ...
            switch (Filter)
            {
                ...
                case Filter.Score:
                {
                    ...
                    foreach (VvVGuildStats stats in list.OrderBy(e => -e.Score))
                    {
                        list.Add(stats);
                    }
                ...
                {
                    list = new List<VvVGuildStats>();
                    foreach (VvVGuildStats stats in list.OrderBy(e => -e.Kills))
                    {
                        list.Add(stats);
                    }
                ...
                {
                    list = new List<VvVGuildStats>();
                    foreach (VvVGuildStats stats in list.OrderBy(e => -e.ReturnedSigils))
                    {
                        list.Add(stats);
                    }
