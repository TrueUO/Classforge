...
namespace Server.Regions
{
    public class WellOfSoulsRegion : Region
    {
        ...
        {
            WellOfSoulsRegion wellOfSoulsRegion = new WellOfSoulsRegion();
        }
    ...
    public class WellOfSoulsVirtuesRegion : Region
    {
        public static void Initialize()
        {
            WellOfSoulsVirtuesRegion virtueRegion = new WellOfSoulsVirtuesRegion();
        }
        ...
        {
            VirtueDef virtue = Virtue.FirstOrDefault(x => x.Area.Contains(m.Location));

            if (virtue != null)
            {
                RewardArray list = JollyRogerData.GetList(m);

                if (list != null && list.Shrine != null)
                {
                    ShrineArray s = list.Shrine.FirstOrDefault(x => x.Shrine == virtue.Shrine);

                    if (s != null && s.MasterDeath >= 3)
                    {
                        if (!list.Cloak && list.Shrine.Count == 8 && !list.Shrine.Any(x => x.MasterDeath < 3))
                        {
                            CloakOfTheVirtuous item = new CloakOfTheVirtuous();
