...
namespace Server.Engines.JollyRoger
{
    ...
    public class ShrineBattleController : Item
    {
        ...
        public void BeginInvasion()
        {
            ...
            for (int index = 0; index < Defs[Shrine].Length; index++)
            {
                Rectangle2D rectangle2D = Defs[Shrine][index];
        ...
        public void OnMasterDestroyed()
        {
            ...
            foreach (KeyValuePair<BaseCreature, List<BaseCreature>> x in Spawn)
            {
                if (x.Key.Alive)
                {
                    any = true;
        ...
        public bool MasterBlessCheck(ShrineMaster master)
        {
            ...
            for (int index = 0; index < Spawn[master].Count; index++)
            {
                BaseCreature x = Spawn[master][index];
