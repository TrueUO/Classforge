    ...
    public abstract class HarvestSystem
    {
        ...
        public virtual void FinishHarvesting(Mobile from, Item tool, HarvestDefinition def, object toHarvest, object locked)
        {
            ...
            if (CheckHarvestSkill(map, loc, from, resource, def))
            {
                ...
                if (tool is IUsesRemaining withUses && (withUses is BaseHarvestTool || withUses is Pickaxe || withUses is SturdyPickaxe || withUses is GargoylesPickaxe))
                {
...
namespace Server
{
    ...
    public class FurnitureAttribute : Attribute
    {
        ...
        {
            for (int index = 0; index < _NotChoppables.Length; index++)
            {
                Type t = _NotChoppables[index];
