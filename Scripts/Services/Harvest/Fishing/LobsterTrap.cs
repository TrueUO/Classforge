...
namespace Server.Items
{
    ...
    public class LobsterTrap : Item, IBaitable
    {
        ...
        public void OnTarget(Mobile from, object targeted)
        {
            ...
            {
                bool lava = this is LavaLobsterTrap;

                if (IsValidTile(from, targeted, lava))
                {
                    from.RevealingAction();

                    LobsterTrapMechanism mechanism = new LobsterTrapMechanism(from, BaitType, EnhancedBait);
        ...
        public static bool IsNotShallowWaterStaticTile(Map map, IPoint3D pnt)
        {
            ...
            Misc.Geometry.Circle2D(new Point3D(pnt.X, pnt.Y, pnt.Z), map, 5, (p, m) =>
            {
                ...
                if (stile.Length > 0)
                {
                    for (int index = 0; index < stile.Length; index++)
                    {
        ...
        {
            int[] tiles = Engines.Harvest.Fishing.WaterTiles;
        ...
        {
            int[] tiles = Engines.Harvest.Fishing.LavaTiles;
