...
namespace Server.Items
{
    public class FishInfo
    {
        ...
        public static void InvalidateLocations()
        {
            ...
            for (int index = 0; index < m_FishInfos.Count; index++)
            {
        ...
        public static Type GetSpecialItem(Mobile from, Item harvestItem, Point3D pnt, double bump, bool rareOnly)
        {
            ...
            foreach (FishInfo info in infos.OrderByDescending(i => i.Type == bait))
            {
        ...
        {
            List<FishInfo> list = new List<FishInfo>();
            Map fisherMap = fisher.Map;
            Point3D fisherLoc = fisher.Location;

            for (int i = 0; i < m_FishInfos.Count; i++)
            {
                FishInfo info = m_FishInfos[i];
        ...
        {
            double baitStr = GetBaitStrength(bait, from, enhanced);
