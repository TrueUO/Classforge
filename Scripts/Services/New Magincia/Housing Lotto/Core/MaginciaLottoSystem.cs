    ...
    public class MaginciaLottoSystem : Item
    {
        ...
        public static void SendMessageTo(Mobile from, NewMaginciaMessage message)
        {
            ...
            if (from is PlayerMobile mobile && mobile.NetState != null)
            {
                ...
                List<NewMaginciaMessage> messages = GetMessages(mobile);
        ...
        public static void OnLogin(Mobile from)
        {
            ...
            List<NewMaginciaMessage> messages = GetMessages(from);
        ...
        public static List<NewMaginciaMessage> GetMessages(Mobile m)
        {
            ...
            {
                List<NewMaginciaMessage> messages = m_MessageQueue[m];
            ...
            if (m.Account != null)
            {
                foreach (KeyValuePair<Mobile, List<NewMaginciaMessage>> kvp in m_MessageQueue.Where(kvp => kvp.Key != m && kvp.Key.Account != null && kvp.Key.Account.Username == m.Account.Username && kvp.Value.Any(message => message.AccountBound)))
                {
                    if (list == null)
                    {
                        list = new List<NewMaginciaMessage>();
        ...
        public static void GetWinnerGump(Mobile from)
        {
            ...
            for (int i = 0; i < acct.Length; i++)
            {
                ...
                for (int index = 0; index < m_Plots.Count; index++)
                {
