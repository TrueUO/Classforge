...
namespace Server.Engines.Shadowguard
{
    public class BarEncounter : ShadowguardEncounter
    {
        ...
        public override void CheckEncounter()
        {
            ...
            for (int index = 0; index < Bottles.Count; index++)
            {
                ShadowguardBottleOfLiquor b = Bottles[index];
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            if (Pirates != null)
            {
                for (int index = 0; index < Pirates.Count; index++)
                {
                    Mobile p = Pirates[index];
                    writer.Write(p);
                }
            }
    ...
    public class OrchardEncounter : ShadowguardEncounter
    {
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            if (Trees != null)
            {
                for (int index = 0; index < Trees.Count; index++)
                {
                    ShadowguardCypress t = Trees[index];
                    writer.Write(t);
                }
            }
    ...
    public class ArmoryEncounter : ShadowguardEncounter
    {
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            if (DestroyedArmor != null)
            {
                for (int index = 0; index < DestroyedArmor.Count; index++)
                {
                    Item a = DestroyedArmor[index];
                    writer.Write(a);
                }
            }
    ...
    public class FountainEncounter : ShadowguardEncounter
    {
        ...
        public void UseSpigot(ShadowguardSpigot spigot, Mobile m)
        {
            ...
            for (int index = 0; index < FlowCheckers.Count; index++)
            {
        ...
        public override void CheckEncounter()
        {
            ...
            if (FlowCheckers != null)
            {
                for (int index = 0; index < FlowCheckers.Count; index++)
                {
                    FlowChecker c = FlowCheckers[index];
        ...
        public class FlowChecker
        {
            ...
            public bool Connects(Item one, Item two)
            {
                if (one is ShadowguardCanal canal && two is ShadowguardDrain)
                {
                    ...
                    switch (d)
                    {
                        ...
                        {
                            canConnect = flow == Flow.NorthSouth || flow == Flow.SouthEastCorner || flow == Flow.SouthWestCorner; break;
                        }
                        case Direction.East:
                        {
                            canConnect = flow == Flow.EastWest || flow == Flow.NorthWestCorner || flow == Flow.SouthWestCorner; break;
                        }
                        case Direction.South:
                        {
                            canConnect = flow == Flow.NorthSouth || flow == Flow.NorthEastCorner || flow == Flow.NorthWestCorner; break;
                        }
                        case Direction.West:
                        {
                            canConnect = flow == Flow.EastWest || flow == Flow.NorthEastCorner || flow == Flow.SouthEastCorner; break;
                        }
                    }
            ...
            private void Fill()
            {
                ...
                {
                    ShadowguardCanal i = _Checked[index];
            ...
            public void Serialize(GenericWriter writer)
            {
                ...
                if (_Checked != null)
                {
                    for (int index = 0; index < _Checked.Count; index++)
                    {
                        ShadowguardCanal c = _Checked[index];
                        writer.Write(c);
                    }
                }
            }
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            if (Elementals != null)
            {
                ...
                {
                    BaseCreature e = Elementals[index];
                    writer.Write(e);
                }
            }
            ...
            if (ShadowguardCanals != null)
            {
                for (int index = 0; index < ShadowguardCanals.Count; index++)
                {
                    Item c = ShadowguardCanals[index];
                    writer.Write(c);
                }
            }
            ...
            if (FlowCheckers != null)
            {
                for (int index = 0; index < FlowCheckers.Count; index++)
                {
                    FlowChecker f = FlowCheckers[index];
                    f.Serialize(writer);
                }
            }
        }
    ...
    public class BelfryEncounter : ShadowguardEncounter
    {
        ...
        public void SpawnDrake(Point3D p, Mobile from)
        {
            ...
            for (int index = 0; index < SpawnRecs.Length; index++)
            {
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            if (Drakes != null)
            {
                for (int index = 0; index < Drakes.Count; index++)
                {
                    VileDrake d = Drakes[index];
                    writer.Write(d);
                }
            }
            ...
            if (Bells != null)
            {
                for (int index = 0; index < Bells.Count; index++)
                {
                    Item b = Bells[index];
                    writer.Write(b);
                }
