...
namespace Server.Items
{
    public class ExodusSummoningAlter : BaseDecayingItem
    {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                if (targeted is StaticTarget staticTarget)
                {
                    if (IsValidTile(staticTarget.ItemID) && (from.Map == Map.Felucca || from.Map == Map.Trammel))
                    {
                        ...
                        foreach (Item x in from.Map.GetItemsInRange(staticTarget.Location, 5))
                        {
        ...
        public static bool CheckExodus() // Before ritual check
        {
            ...
            for (int index = 0; index < ClockworkExodus.Instances.Count; index++)
            {
                ClockworkExodus m = ClockworkExodus.Instances[index];
