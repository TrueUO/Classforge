...
namespace Server.Engines.Blackthorn
{
    ...
    public class InvasionController : Item
    {
        ...
        public void BeginInvasion()
        {
            ...
            for (int index = 0; index < Defs[CurrentInvasion].SpawnRecs.Length; index++)
            {
                Rectangle2D rec = Defs[CurrentInvasion].SpawnRecs[index];
        ...
        public void OnDeath(BaseCreature bc)
        {
            ...
            else
            {
                foreach (KeyValuePair<BaseCreature, List<BaseCreature>> kvp in Spawn)
                {
                    ...
                    for (int index = 0; index < kvp.Value.Count; index++)
                    {
                        BaseCreature b = kvp.Value[index];
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            if (SpawnZones != null)
            {
                for (int index = 0; index < SpawnZones.Count; index++)
                {
                    Rectangle2D rec = SpawnZones[index];
